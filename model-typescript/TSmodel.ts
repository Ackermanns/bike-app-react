...
class Bike {
    ...
    addPart(newname:string="apart", newdateinstalled:Date=new Date(), newweight:number=1) {
        //Creates a part object that stays within the Bike class

        const newPart = new Part(this.partcount, newname, newdateinstalled, newweight);
    ...
    filterParts(search:string):Part[] {
    ...
    deletePart(pid:number) {
        //Deletes the part object with the given name
        //NOTE: will delete duplicate entries
        let index: number = 0;
    ...
    createJSONParts():string {
        //creates the parts objects in a string JSON format
        let obj: string = ''
        let partsJSON: string = '{'
        for (let p of this.partslist) {
            obj = '"'+p.id+'":{"id":'+p.id+' "name":"'+p.name+'", "dateInstalled":"'+p.dateInstalled+'", "weight":'+p.weight+', "age":"'+p.age+'"},'
    ...
    savePartsToLocal(JSONstring:string):boolean {
    ...
    loadPartsFromLocal() {
        //Get all objectsfrom localstorage
        //Since localstorage will only be used for storing bike parts we edo not need further processing
        //NOTE: any type was the only way to get it to work properly
        const StringObj: any = localStorage.getItem('BikeParts');
        const jsonObj: any = JSON.parse(StringObj);
        let storage = []
        for (let i = 0; i < this.partcount; i++) {
            storage.push(jsonObj[i.toString()]);
            //Anything else needing to be done with the storage will be done here
            //May be useful for assignment 3 for further processing to be done here
        }
        return storage;
    }
    specificSearchPart(searchname:string):Part {
...
class Part {
    ...
    //restore variables
    restorename: string
    restoredateInstalled: string | Date
    restoreweight: number
    //Note for bad date value will automatically stamp todays date object in the validator
    constructor(newid:number, newname:string="apart", newdateinstalled:Date=new Date(), newweight:number=1) {
        ...
        this.restorename = newname;
        this.restoredateInstalled = newdateinstalled;
        this.restoreweight = newweight;
    }

    constructorValidator(newname:string, newdateinstalled:Date, newweight:number):any[] {
    ...
    changeDate(newdateinstalled:Date) {
    ...
    checkAge():string {
        ...
        let date1:Date = new Date(); //todays date
        let date2:Date = new Date(this.dateInstalled) // already a date object
        //Date difference
        let difference:number = date1.getTime() - date2.getTime();
        //Days between two dates
        let dayDifference:number = difference / (1000 * 3600 * 24);
